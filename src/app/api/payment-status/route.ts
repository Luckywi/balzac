...
export async function GET(request: Request) {
  try {
    ...
    // Vérifier si un RDV correspondant existe dans Firebase
    const rdvsCollection = collection(db, "rdvs");
    const rdvQuery = query(rdvsCollection, where("paymentIntentId", "==", paymentId));
    const rdvSnapshot = await getDocs(rdvQuery);
    
    if (!rdvSnapshot.empty) {
      // Si un RDV existe, on récupère son statut
      const rdv = rdvSnapshot.docs[0];
      const rdvData = rdv.data();
      
      return NextResponse.json({
        status: rdvData.paymentStatus || 'succeeded',
        bookingId: rdv.id,
        data: rdvData
      });
    }
    
    // Si aucun RDV n'est trouvé, on vérifie directement auprès de Stripe
    try {
      ...
      if (paymentIntent.status === 'succeeded') {
        return NextResponse.json({
          status: 'pending',
          message: 'Paiement confirmé par Stripe, mais traitement en cours'
        });
